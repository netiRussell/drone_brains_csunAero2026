 ...
 * 
 * Modified by Rulsan Abdulin in December 2025 to ensure the NEC protocol is followed correctly
 * Original version expects command and address to have the inverses included which is not user-friendly.
 * 
*/
...
static size_t rmt_encode_ir_nec(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    // fall-through
    case 1: // send commands and address
        // - Construct the payload -
        // Note: (uint8_t) is added to keep the inverse width-limited due to Integer promotion 
        uint8_t nec_payload[4] = {
            scan_code->address,             // Address
            (uint8_t)~scan_code->address,   // Inverse of address
            scan_code->command,             // Command
            (uint8_t)~scan_code->command    // Inverse of command
        };

        // - Apply the payload and change states -
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, &nec_payload, sizeof(nec_payload), &session_state);
        if (session_state & RMT_ENCODING_COMPLETE) {
